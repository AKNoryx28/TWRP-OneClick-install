...
namespace Tools.Fastboot
{
    /// <summary>
    /// The main of Fastboot class
    ...
    {
        private const ushort USB_VID = 0x18D1;
        private const ushort USB_PID = 0xD00D;
        ...
        public Fastboot(string serial) { targetSerialNumber = serial; }
        public Fastboot() { targetSerialNumber = null; }
        ...
        /// <summary>
        /// Wait for Fastboot device to connect.
        /// </summary>
        public void Wait(int timeout=50)
        {
            var counter = 0;

            while (counter < timeout)
            {
                // Get all devices from usb
                var allDevices = UsbDevice.AllDevices;

                if (allDevices.Any(x => x.Vid == USB_VID & x.Pid == USB_PID)) return;
                if (counter == timeout) throw new Exception("Timeout error.");
        ...
        public void Connect()
        {
            ...
            if (device == null) throw new Exception("No devices available.");
        ...
        public void Disconnect() { 
            ...
            else throw new Exception("No devices connected");
        }
        ...
        {
            if (device == null)
                return new Response(Status.Fail, string.Empty);
            ...
            if (wrAct != command.Length)
                throw new Exception($"Failed to write command! Transferred: {wrAct} of {command.Length} bytes");
